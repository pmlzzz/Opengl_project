...
glm::mat4 proj=glm::perspective(70.0f,//fovy
				  		       1.0f,//aspect
						        0.01f,1000.f); //near, far
float color[4] = { 1.0,1.0,0.0,0.0 };
float color1[4] = { 1.0,0.0,0.0,0.0 };
...
glm::mat4 Arm(glm::mat4 m, GLfloat d)
{
//let's use instancing
	glm::mat4 next;
	//m=glm::translate(m,glm::vec3(0,0.5,0.0));
	glm::mat4 tmp=m;
	m = glm::rotate(m, -10.0f*ftime, glm::vec3(0.0, 1.0, 0.0));
	
	sphere->SetMVP(glm::scale(m,glm::vec3(1.1f,1.0f,1.1f)));
	sphere->Render();
	m = tmp;
	//m = glm::rotate(m, -10.0f*ftime, glm::vec3(0.0, 1.0, 0.0));
	cube->SetMVP(glm::scale(m, glm::vec3(100.1f, 1.0f, 100.1f)));
	cube->Render();
	//sphere->SetMVP(glm::scale(m, glm::vec3(0.1f, 1.0f, 0.1f)));
	//sphere->Render();C
...
//optimized
    ...
    //printf("asdasdasd %d", (rand()%10));
	next = m;
	m=glm::rotate(m,-100.0f*ftime,glm::vec3(0.0,1.0, 0.0));
	m = glm::translate(m, glm::vec3(2.0, 0.0, 0.0)); 
	//m=glm::translate(m,glm::vec3(0.0,0.5,0.0));
	sphere->SetMVP(glm::scale(m,glm::vec3(0.1f*d,0.1f*d,0.1f*d)));
	glUniform4fv(colorParameter, 1, color1);
	sphere->Render();
	m = next;
	m = glm::rotate(m, -90.0f*ftime, glm::vec3(0.0, 1.0, 0.0));
	...
	m = next;
	m = glm::rotate(m, -80.0f*ftime, glm::vec3(0.0, 1.0, 0.0));
	...
	m = next;
	m = glm::rotate(m, -70.0f*ftime, glm::vec3(0.0, 1.0, 0.0));
	m = glm::translate(m, glm::vec3(5.0, 0.0, 0.0));
	//m=glm::translate(m,glm::vec3(0.0,0.5,0.0));
	sphere->SetMVP(glm::scale(m, glm::vec3(0.3f*d, 0.3f*d, 0.3f*d)));
	sphere->Render();

	m = glm::rotate(m, -200.0f*ftime, glm::vec3(5.0, 0.0, 0.0));
	...
	m = next;
	m = glm::rotate(m, -60.0f*ftime, glm::vec3(0.0, 0.0, 5.0));
	m = glm::translate(m, glm::vec3(0.0, 6.0, 0.0));
	//m=glm::translate(m,glm::vec3(0.0,0.5,0.0));
	sphere->SetMVP(glm::scale(m, glm::vec3(0.5f*d, 0.5f*d, 0.5f*d)));
	sphere->Render();
	m = glm::rotate(m, -200.0f*ftime, glm::vec3(5.0, 0.0, 0.0));
	...
	m = next;
	m = glm::rotate(m, -50.0f*ftime, glm::vec3(0.0, 0.0, 5.0));
	m = glm::translate(m, glm::vec3(0.0, 7.0, 0.0));
	//m=glm::translate(m,glm::vec3(0.0,0.5,0.0));
	sphere->SetMVP(glm::scale(m, glm::vec3(0.5f*d, 0.5f*d, 0.5f*d)));
	sphere->Render();

	m = next;
	m = glm::rotate(m, -40.0f*ftime, glm::vec3(0.0, 0.0, 5.0));
	m = glm::translate(m, glm::vec3(0.0, 8.0, 0.0));
	//m=glm::translate(m,glm::vec3(0.0,0.5,0.0));
	sphere->SetMVP(glm::scale(m, glm::vec3(0.2f*d, 0.2f*d, 0.2f*d)));
	sphere->Render();

	m = next;
	m = glm::rotate(m, -30.0f*ftime, glm::vec3(0.0, 0.0, 5.0));
	m = glm::translate(m, glm::vec3(0.0, 9.0, 0.0));
	//m=glm::translate(m,glm::vec3(0.0,0.5,0.0));
	sphere->SetMVP(glm::scale(m, glm::vec3(0.2f*d, 0.2f*d, 0.2f*d)));
	sphere->Render();
...
void RenderObjects()
{
	...
	view=glm::lookAt(glm::vec3(15*sin(-ftime/5.f),5.f+pml,15*cos(-ftime /5.f)),//eye
	...
	glm::mat4 m = glm::translate(glm::mat4(1.0), glm::vec3(1, 0, 1));
	Arm(m,1);
