...
void RenderObjects()
{
	...
	//we know shapes[0] and shapes[1] are the two cubes
	glm::mat4 m=glm::mat4(1.0);
//	m=m*glm::rotate(glm::mat4(1.0),0.5f*ftime,glm::vec3(0.0,1.0,0.0));
	m=glm::rotate(m,ftime,glm::vec3(0,0,1));
	m=glm::translate(m,glm::vec3(0,0.5,0));
	m=glm::scale(m,glm::vec3(0.1,1,0.1));
	shapes[0]->SetMVP(m);
	shapes[0]->Render();

	m=glm::scale(m,glm::vec3(10.f,1.0f,10.f));
	m=glm::translate(m,glm::vec3(0.0,0.5,0.0));
	m=glm::rotate(m,-20.0f*ftime,glm::vec3(0.0,0.0,1.0));
	m=glm::translate(m,glm::vec3(0.0,0.5,0.0));
	m=glm::scale(m,glm::vec3(0.1f,1.0f,0.1f));
	shapes[1]->SetMVP(m);
	shapes[1]->Render();
}
...
{
//create just two unit cubes in the origin
	ShapesC *tmp;
	tmp=new CubeC();
