...
vec4 lighting(vec3 pos, vec3 rayDir)
{
	...
	float f=pow((1.0-Eta)/(1.0+Eta),2.0);
	float F = f+(1.0-f)*pow((1.0-dot(n, v)),5);

	//m=0.5;
	float k=pow(dot(n, h),2);
	float D=exp((k-1)/(k*pow(m,2)))	/(4*pow(m,2)*pow(k,2));

	float G=min(1,min(2*dot(n, h)*dot(n, v)/dot(v, h),2*dot(n, h)*dot(n, light)/dot(v, h)));
	
	//vec4 I=ambient_color + diffuse_color*max(0.0, dot(n, light)) + spec_color*pow(max(0.0, dot(r, v)), 50.0);
	vec4 I=ambient_color + diffuse_color*max(0.0, dot(n, light)) + spec_color*F*D*G/(3.1415926*dot(n, v));
	if(option2==0)
	{
		I=ambient_color + diffuse_color*max(0.0, dot(n, light)) + spec_color*F*D*G/(3.1415926*dot(n, v));
	}
	else if (option2==1)
	{
		I=ambient_color + diffuse_color*max(0.0, dot(n, light)) + spec_color*F/(3.1415926*dot(n, v));
	}
	else if (option2==2)
	{
		I=ambient_color + diffuse_color*max(0.0, dot(n, light)) + spec_color*D/(3.1415926*dot(n, v));
	}
	else if (option2==3)
	{
		I=ambient_color + diffuse_color*max(0.0, dot(n, light)) + spec_color*G/(3.1415926*dot(n, v));
	}
	return I;	
}
